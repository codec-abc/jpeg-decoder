...
use huffman::{HuffmanDecoder, HuffmanTable, HuffmanTableClass};
use marker::Marker;
...
impl<R: Read> Decoder<R> {
    ...
    fn decode_scan(&mut self,
        ...
        // Verify that all required huffman tables has been set or created the default one if needed.
        ...
        if scan.spectral_selection.start == 0 &&
            ...
            self.dc_huffman_tables[0] = Some(result.unwrap());

            let result2 = HuffmanTable::new(
            ...
            self.dc_huffman_tables[1] = Some(result2.unwrap());
        }
        ...
        if scan.spectral_selection.end > 1 &&
            ...
            let result =  HuffmanTable::new(
                ...
                &[
                    ...
                    0x05, 0x12, 0x21, // 5 -> 3
                    0x31, 0x41, // 6 -> 2
                    0x06, 0x13, 0x51, 0x61, // 7 -> 4
                    0x07, 0x22, 0x71, // 8 -> 3
                    0x14, 0x32, 0x81, 0x91, 0xA1, // 9 -> 5
                    0x08, 0x23, 0x42, 0xB1, 0xC1, // 10 -> 5
                    0x15, 0x52, 0xD1, 0xF0, // 11 -> 4
                    0x24, 0x33, 0x62, 0x72, // 12 -> 4
                    // 13 -> 0
                    // 14 -> 0
                    0x82,  // 15 -> 1
                    ...
                    0xE8, 0xE9, 0xEA, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA // 16 -> 125
                ], HuffmanTableClass::AC);

            self.ac_huffman_tables[0] = Some(result.unwrap());
            ...
            let result2 =  HuffmanTable::new(
                ...
                &[
                    ...
                    0x02, // 3 -> 1
                    0x03, 0x11, // 4 -> 2
                    0x04, 0x05, 0x21, 0x31, // 5 -> 4
                    0x06, 0x12, 0x41, 0x51, // 6 -> 4
                    0x07, 0x61, 0x71, // 7 -> 3
                    0x13, 0x22, 0x32, 0x81, // 8 -> 4
                    0x08, 0x14, 0x42, 0x91, 0xA1, 0xB1, 0xC1, // 9 -> 7
                    0x09, 0x23, 0x33, 0x52, 0xF0, // 10 -> 5
                    0x15, 0x62, 0x72, 0xD1, // 11 -> 4
                    0x0A, 0x16, 0x24, 0x34, // 12 -> 4
                    // 13 -> 0
                    0xE1, // 14 -> 1
                    0x25, 0xF1, // 15 -> 2
                    ...
                    0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA // 16 -> 119
                ] , HuffmanTableClass::AC);

            self.ac_huffman_tables[1] = Some(result2.unwrap());
        }
